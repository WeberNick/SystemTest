...
void MemoryAccess::fPrintResult(std::ostream& output, std::ostream& compare)
{
	output << "sequential/random read: " << std::endl
		...
		<< "             r1/r2: " << fFormatNumber(_r1) << " =?= " << fFormatNumber(_r2) << "\n" <<std::endl;

	compare << "sequential read [ns]/" << ((_x1 / _n) * NS) << std::endl
		<< "random read [ns]/" << ((_x2 / _n) * NS) << std::endl
		<< "factor/" << (_x2 / _x1) << std::endl;
}

void MemoryAccess::fRunTest(std::ostream& output, std::ostream& compare)
{
	int* lId   = new int[_n];
